        ...
        >
            <div
                ...
                <StepContentWrapper
                    ...
                    currentStep={currentStep}
                    direction={direction}
                    className={`space-y-2 px-6 md:px-12 pt-4 ${contentClassName}`}
                >
                    {stepsArray[currentStep - 1]}
                </StepContentWrapper>

                {!isCompleted && (
                    <div className={`px-6 md:px-12 pb-8 md:pb-12 ${footerClassName}`}>
                        <div className={`mt-10 flex ${currentStep !== 1 ? 'justify-between' : 'justify-end'}`}>
                            {currentStep !== 1 && (
                                <button
                                    onClick={handleBack}
                                    className={`cursor-target duration-350 rounded px-4 py-2 transition ${currentStep === 1
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators = false }: StepIndicatorProps) {
    ...
    return (
        ...
        >
            <motion.div
                variants={{
                    inactive: { scale: 1, backgroundColor: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.4)', border: '1px solid rgba(255,255,255,0.1)' },
                    active: { scale: 1.1, backgroundColor: 'var(--primary)', color: '#fff', border: 'none' },
                    complete: { scale: 1, backgroundColor: 'rgba(255,255,255,0.1)', color: 'var(--primary)', border: '1px solid var(--primary)' }
                }}
