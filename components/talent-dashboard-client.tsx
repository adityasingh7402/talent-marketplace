        ...
        <div className="relative min-h-screen w-full bg-background overflow-hidden selection:bg-primary/20">
            ...
            <div className="relative z-10 flex h-screen overflow-hidden">
                    ...
                    {isSidebarOpen && (
                        ...
                        >
                            ...
                            <div className="mt-8 space-y-4 pt-4 border-t border-white/5">
                                <div className="p-4 rounded-xl bg-zinc-900 border border-white/5 flex items-center gap-3 text-left">
                ...
                <main className="grow flex flex-col h-screen overflow-hidden relative">
                    ...
                    <header className="flex items-center justify-between p-4 md:p-6 border-b border-white/5 md:border-none">
                        {!isSidebarOpen && (
                            <button onClick={() => setIsSidebarOpen(true)} className="p-2.5 rounded-xl bg-white/5 border border-white/10 text-white hover:bg-white/10 transition-all cursor-target">
                                <Menu className="w-5 h-5" />
                            </button>
                        )}
                        <div className="ml-auto flex items-center gap-3">
                            <ModeToggle />
                            <div className="px-3 py-1.5 rounded-full border border-primary/20 bg-primary/5 text-primary text-[9px] font-black uppercase tracking-[0.2em] flex items-center gap-2">
                                <span className="w-1 h-1 rounded-full bg-primary animate-pulse" />
                                <span className="hidden xs:inline">Signal Active</span>
                                <span className="xs:hidden">Active</span>
                            </div>
                        </div>
                    </header>

                    <div className="grow overflow-y-auto custom-scrollbar px-4 md:px-10 pb-10" data-lenis-prevent>
                        <div className="max-w-380 mx-auto mt-5">
                            <header className="mb-8 md:mb-12">
                                <motion.h1
                                    initial={{ opacity: 0, y: -20 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    className="text-3xl md:text-5xl font-heading font-black tracking-tighter text-white mb-2 leading-tight"
                                >
