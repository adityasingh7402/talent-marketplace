...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
    ...
    useLayoutEffect(() => {
        const ctx = gsap.context(() => {
            ...
            // Ensure the panels are visible but positioned offscreen by GSAP
            gsap.set([panel, ...preLayers], { visibility: 'visible', opacity: 1 });
        });
        return () => ctx.revert();
            ...
            >
                ...
                >
                    ...
                    <div className="flex items-center gap-4 pointer-events-auto">
                        ...
                        <button
                            ref={toggleBtnRef}
                            className={`sm-toggle cursor-target relative inline-flex items-center gap-[0.3rem] bg-transparent border-0 cursor-pointer font-medium leading-none overflow-visible transition-colors duration-300 ${open ? 'text-black' : 'text-foreground'
                                }`}
                ...
                <aside
                    ...
                    ref={panelRef}
                    className="staggered-menu-panel absolute top-0 right-0 h-dvh bg-white flex flex-col p-[6em_1.5em_1.5em_1.5em] md:p-[6em_2em_2em_2em] overflow-y-auto z-10 backdrop-blur-md pointer-events-auto"
                        ...
                        >
                            {items && items.length ? (
                                ...
                                <li className="sm-panel-itemWrap relative overflow-hidden leading-none" aria-hidden="true">
                                    <span className="sm-panel-item relative text-black font-semibold text-[2.5rem] md:text-[4rem] cursor-pointer leading-none tracking-[-2px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]">
                                        <span className="sm-panel-itemLabel inline-block origin-[50%_100%] will-change-transform">
                                            No items
                                        </span>
                                    </span>
...
.sm-scope .staggered-menu-header { position: absolute; top: 0; left: 0; width: 100%; display: flex; align-items: center; justify-content: space-between; padding: 1.1em 1.5em; background: rgba(var(--background), 0.8); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border-bottom: 1px solid rgba(0,0,0,0.05); pointer-events: none; z-index: 20; }
